...
- `.env` existe, esta ignorado y no esta rastreado. Los valores locales se trasladaron desde `.env.example`; el ejemplo se restauro sin secretos antes de cualquier commit.
...
- Confirmar con el administrador que todos los demas consumidores autorizados tambien fueron actualizados; la revocacion en este destino ya fue verificada abajo.
- Conservar las credenciales configuradas solo en el entorno local o almacen seguro.
- Completar CRUD con una sesion autenticada y datos de prueba autorizados; el arranque y la lectura ya fueron verificados abajo.
