...
$toolDirectory = Join-Path $projectRoot 'obj/DatabaseVerification'
...
"@
[System.IO.File]::WriteAllText((Join-Path $toolDirectory 'DatabaseVerification.csproj'), $projectText)
Copy-Item -LiteralPath (Join-Path $PSScriptRoot 'Verificacion.cs.txt') -Destination (Join-Path $toolDirectory 'Program.cs') -Force
dotnet run --project (Join-Path $toolDirectory 'DatabaseVerification.csproj') -- $projectRoot
if ($LASTEXITCODE -ne 0) { throw 'Fallo la verificacion de MySQL; revisa el resultado anterior.' }
